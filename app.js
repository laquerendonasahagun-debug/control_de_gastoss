...
function conceptDetailHtml(categoryId, categoryName, detailId) {
  ...
  const rows = entries.map(entry => `<tr><td class="date-cell concept-detail-date" data-label="Fecha">${shortDate(entry.date)}</td><td class="concept-detail-spender" data-label="Realizó el gasto">${escapeHtml(entry.spender || '—')}</td><td class="note-cell concept-detail-note" data-label="Nota">${escapeHtml(entry.note || 'Sin nota')}</td><td class="align-right amount-cell concept-detail-amount" data-label="Monto">${money(entry.amount)}</td></tr>`).join('');
  return `<div class="concept-detail" id="${detailId}"><div class="concept-detail-heading"><div><p class="eyebrow">Detalle</p><h4>Movimientos de ${escapeHtml(categoryName)}</h4></div><span class="panel-caption">${countLabel}</span></div>${entries.length ? `<div class="table-scroll concept-detail-scroll"><table><thead><tr><th>Fecha</th><th>Realizó el gasto</th><th>Nota</th><th class="align-right">Monto</th></tr></thead><tbody>${rows}</tbody></table></div>` : '<div class="empty-row">No hay movimientos de este concepto en el rango seleccionado.</div>'}</div>`;
}
